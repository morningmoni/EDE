...
def _sequential_topk(beam_size,
    ...
    for row, col, seq_score in zip(best_ids, best_word_ids, sequence_scores):
        ...
        if hypotheses[row] is not None and hypotheses[row].is_valid(col):
            ...
            if debug:
                print(f'{(row, col), scores[row, col]} added because overall best and is_valid')
    ...
    best_next = torch.argmax(scores, dim=1)  # NB argmax since we use log_probs as scores
    for row in range(beam_size):
        ...
        if debug:
            for col in nextones:
                print(f'{(row, col), scores[row, col]} added because extend constraint')

        # (3) add the single-best item after this (if it's valid)
        col = int(best_next[row].item())
        if hyp.is_valid(col):
            if debug:
                print(f'{(row, col), scores[row, col]} added because best-next')
            nextones.add(col)
        ...
        for col in nextones:
            ...
            if score > -1e10:  # 1. at the 1st step other beams are -inf 2. when < MIN_LEN, EOS is -1e20
                cand = ConstrainedCandidate(row, col, score, new_item)
                candidates.add(cand)
            # else:
            #     print(f'{(row, col), tokenizer.convert_ids_to_tokens(col), score} NOT added')
